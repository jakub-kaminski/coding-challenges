// QUESTION NAME: SBS0001 Find the first occurence of x in a sorted array
...
class Solution {
public:
    static int firstOccurrence(vector<int> v, int t){
       int ind = lower_bound(v.begin(), v.end(), t) - v.begin();
       if(ind != v.size() && v[ind] == t) return ind;
       else return -1;
    }
};
...
//@formatter:on
    ...
    SECTION(msg) {
        ...
        auto result = solver.firstOccurrence(v, t);
        CAPTURE(result);
